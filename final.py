...
pd.read_sql("SELECT country, shortName, name, activityType, ecContribution, organizationURL, SUM(ecContribution), COUNT(organizationURL) FROM participants WHERE role='coordinator' GROUP BY organizationURL ORDER BY SUM(ecContribution) DESC",conn)
