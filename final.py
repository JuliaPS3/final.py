...
together=pd.read_sql("SELECT shortName, name, activityType, ecContribution, organizationURL, role, countries.Country FROM Participants LEFT JOIN countries ON participants.country=countries.Acronym", conn)
c1=pd.read_sql("SELECT country, shortName, name, activityType, ecContribution, organizationURL FROM together WHERE role = 'coordinator' AND Country='FR'",conn)
