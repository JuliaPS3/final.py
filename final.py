...
c1=pd.read_sql("SELECT country, shortName, name, activityType, ecContribution, organizationURL, COUNT(organizationURL) FROM Participants WHERE role = 'coordinator' AND country={} GROUP BY organizationURL ORDER BY ecContribution DESC".format(country_selected),conn)
