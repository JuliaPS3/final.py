...
c1=pd.read_sql("SELECT country, shortName, name, activityType, SUM(ecContribution), organizationURL, COUNT(organizationURL) FROM participants WHERE role='coordinator' AND country='FR' GROUP BY organizationURL ORDER BY SUM(ecContribution) DESC",conn)
